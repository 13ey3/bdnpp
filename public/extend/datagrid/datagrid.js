var datagrid = function (options) {
    ...
    function loadData() {
        ...
        $.ajax({
            ...
            content.forEach(function (item, index) {
                ...
                Object.values(item).forEach((value, column) => {
                    if (!Array.isArray(value)) {
                        td += '<td style="color:black; font-size:13px" class="' + params.columns[column].class + '">' + value + '</td>';
                    }
                });
                if (params.action.length > 0) {
                    td += '<td class="text-center">';
                    item.actionList.forEach(function (action) {
                        var actionParams = params.action.find(a => a.name == action.actionName);
                        var target = (actionParams.target) ? ' target="' + actionParams.target + '"' : '';
                        var title = actionParams.btnTitle === undefined ? '' : actionParams.btnTitle;
                        if (action.actionActive) {
                            td += '<a href="' + action.actionUrl + '"' + target + '" class="' + actionParams.btnClass + '" title="'+ title +'">' + actionParams.btnText + '</a> &nbsp;';
                        }
                    });
                    td += '</td>';
                }
    ...
    function _generatePaginationFooter() {
        var footer = '<div class="row">' +
            ...
            '<li class="page-item prevPage' + _getTableId() + '"><a class="page-link" href="#">Prev</a></li>' +
            ...
            '<li class="page-item nextPage' + _getTableId() + '"><a class="page-link" href="#">Next</a></li>' +
            '</ul>' +
            '</div>' +
            '</div>';
